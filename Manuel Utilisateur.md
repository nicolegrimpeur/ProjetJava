...
Pour lancer l'application, il suffit de cliquer sur le script de commande "launchApplication.cmd". L'application s'ouvrira ensuite toute seule.

### Interface <a id="interface"></a>
L'interface est répartie en plusieurs parties :
- La **barre de menu**, à partir de laquelle on peut fermer l'application (Cliquer sur `File` puis `Close`) ou obtenir la version de l'application (`Help` puis `A propos`).
...
Pour **modifier un employé**, clique sur l'employé à modifier dans le tableau. Ses informations s'afficheront sur le côté, vous pouvez par ce biais modifier directement le job ou le salaire de l'employé.
Si vous modifiez le nom d'un employé, pensez à supprimer l'ancienne version manuellement.
>Vous ne pouvez pas modifier les données d'un employé en service.


### Réserves <a id="reserves"></a>

L'écran de réserve permet de voir les ingrédients en stock mais aussi de modifier ceux-ci et d'imprimer la liste des ingrédients à ajouter afin de retrouver le stock initial au début de la journée.
...
>Vous pouvez modifier l'apparence de la liste de courses en modifiant le fichier coursesTemplate.html dans le dossier "ressources" de l'application.
...
- Le chiffre d'affaires total de la journée avec et sans TVA
...
- Pour **valider la prise de commande** cliquez sur le bouton `Enregistrer`.
...
- Cliquer sur le nom du serveur en charge de la commande puis sur le bouton `Servi`.
> La commande ne peut être servie que si tous les plats et toutes les boissons de la commande sont prêts, lorsque le statut du serveur est "À servir".

Une fois la commande servie, une facture est générée dans le dossier "Factures", ainsi qu'une addition générée dans le dossier "Additions". Cette dernière peut être imprimé depuis l'écran Addition ci après.
...
L'écran des additions permet l'accès à une liste d'additions afin de noter la commande comme payée. Il est aussi possible de les imprimer individuellement.

Pour l'**afficher**, cliquez sur l'addition, elle s'affichera dans le tableau à droite de la liste des additions.

Pour **payer l'addition**, cliquez sur l'addition puis sur le bouton `Marquer comme payé`. 

Pour l'**imprimer**, cliquez sur l'addition voulue puis sur le bouton `Imprimer`.
...
- Le prix total de la commande avec et sans la TVA.

Il est aussi possible de récupérer les additions dans le dossier de l'application dans le dossier "Additions".
>Vous pouvez modifier l'apparence de l'addition en modifiant le fichier additionTemplate.html dans le dossier "ressources" de l'application.

## Factures <a id="factures"></a>

Les factures sont disponibles dans le dossier "Factures" du dossier de l'application.

Il est nécessaire pour l'entreprise de renseigner ses informations (Nom, n° de téléphone, Sire, n° de TVA, naf) au préalable dans le fichier factureTemplate.html qui se trouve dans le dossier "ressources".